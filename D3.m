...
MM = [Rot_Matrix(1,1) Rot_Matrix(1,2) Rot_Matrix(1,3) Tr(1);
      Rot_Matrix(2,1) Rot_Matrix(2,2) Rot_Matrix(2,3) Tr(2);
      Rot_Matrix(3,1) Rot_Matrix(3,2) Rot_Matrix(3,3) Tr(3);
      0 0 0 1
    ];
...
for x = 1:480
    for y = 1:640
        ...
        %Point3D_R_L = MM_2 * Point3D;
        %IRX_R_L(x, y) = Point3D_R_L(1) / Point3D_R_L(4);
        %IRY_R_L(x, y) = Point3D_R_L(2) / Point3D_R_L(4);
        %IRZ_R_L(x, y) = Point3D_R_L(3) / Point3D_R_L(4);
        
        
        fprintf(fid_1, '%f %f %f %f %f %f\n', IRZ_Left(x, y), (-1)*IRY_Left(x, y), (-1)*IRX_Left(x, y), RGB_Left(x, y, 1), RGB_Left(x, y, 2), RGB_Left(x, y, 3));
        fprintf(fid_1, '%f %f %f %f %f %f\n', IRZ_R_L(x, y), (-1)*IRY_R_L(x, y), (-1)*IRX_R_L(x, y), RGB_Right(x, y, 1), RGB_Right(x, y, 2), RGB_Right(x, y, 3));
        
        fprintf(fid_2, '%f %f %f\n', IRZ_Left(x, y), (-1)*IRY_Left(x, y), (-1)*IRX_Left(x, y));
        fprintf(fid_3, '%f %f %f\n', IRZ_R_L(x, y), (-1)*IRY_R_L(x, y), (-1)*IRX_R_L(x, y));
    end
end
fclose(fid_1);
fclose(fid_2);
